class Game{
    constructor(dartBoard){
        ...
        this.botFinishChances = [0.1, 0.15, 0.20, 0.25, 0.30, 0.35, 0.40, 0.45, 0.50, 0.55]
    ...
    playerThrow(e){
        ...
        let y = e.clientY - canvas.offsetTop - 300
        let x = e.clientX - canvas.offsetLeft - 300
        let newThrow = 0
        if(Math.sqrt((x ** 2 +  y ** 2)) <= 280){
            newThrow = this.dartBoard.boardHit(e.clientX - canvas.offsetLeft - 300, canvas.offsetTop - e.clientY + 300)
        }
    ...
    start(playerName, secondPlayerName, botName, botLevel, legs, sets, points){
        ...
        this.playerTwo = this.bot ? new Player(botName, points, 0, 0, true) : new Player(secondPlayerName, points, 0, 0, false)
    ...
    botTurn(){
        let newThrow = 0
        let unFinishable = {169: true, 168: true, 166: true, 165: true, 163: true, 162: true, 159: true}
        if(this.playerTwo.getPoints() > 170 || unFinishable[this.playerTwo.getPoints()]){
            for(let i=0; i<3; i++){
    ...
    botThrow(){
        return this.botThrwoChances[this.botLevel][Math.floor(Math.random()*this.botThrwoChances[this.botLevel].length)];
    }
