...
let dartBoard = new DartBoard(ctx, 20)
...
function onOffGame(){
    if(game.isRunning()){
        ...
        game.start(playerName.value, secondPlayerName.value, bot.selectedOptions[0].innerText,
            parseInt(bot.value), parseInt(legs.value), parseInt(sets.value), parseInt(gameMode.value))
...
function addAnotherPlayer(){
    secondPlayerName.style.visibility = secondPlayerName.style.visibility == 'visible' ? 'hidden' : 'visible'  
}
